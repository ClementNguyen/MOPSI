...
def createTaggedDocuments(data):
    ...
    for index,row in data.iterrows():
        if (index%10000==0):
            print(index)
        train.append(doc2vec.TaggedDocument(row['Name']+row['Description'],tags=[row['ID']]))
    return train        
...
def trainDoc2Vec():
    ...
    for epoch in range(10):
        print('epoch n°',epoch)
        model.train(utils.shuffle(train),total_examples=len(train),epochs=1)
        model.alpha -= 0.002
        model.min_alpha = model.alpha
    model.save(".\\ml\\word2vec\\d2v_model")
